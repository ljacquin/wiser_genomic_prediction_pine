# functions which corrects for trait spatial heterogeneity for a envir
  ...
  tryCatch(
    {
      ...
      if (nrow(df_envir_) > min_obs_) {
        df_envir_$Lat <- as.factor(df_envir_$Latitude)
        df_envir_$Lon <- as.factor(df_envir_$Longitude)
        df_envir_$Genotype <- as.factor(df_envir_$Genotype)
        spats_formula <- as.formula(~ PSANOVA(Longitude, Latitude))
        spats_model <- SpATS(
          response = trait_,
          spatial = spats_formula,
          genotype = "Genotype", genotype.as.random = TRUE, fixed = NULL,
          random = ~ Lat + Lon, data = df_envir_,
...
# function to recode a genotype table
recode_genotypes <- function(geno_df) {
  # function for each snp (each column)
  recode_snp_column <- function(col) {
    # extract all genotypes from the column (snp)
    # get unique alleles present
    alleles <- unique(unlist(strsplit(col, "")))

    # randomly assign a code 0 or 2 to the two alleles
    recode_map <- sample(c(0, 2))
    names(recode_map) <- alleles

    # function to recode each genotype
    recode_genotype <- function(genotype) {
      allele_pair <- unlist(strsplit(genotype, ""))
      if (allele_pair[1] == allele_pair[2]) {
        # homozygote
        return(recode_map[allele_pair[1]])
      } else {
        # heterozygote
        return(1)
      }
    ...
    # apply the recoding to the entire column
    return(sapply(col, recode_genotype))
  }

  # apply the recoding function to each column (each snp)
  geno_recoded <- apply(geno_df, 2, recode_snp_column)

  # return both the recoded genotypes and the recoding choices
  return(
    as.data.frame(geno_recoded)
  )
}

# function to replace "NN" with the most frequent genotype in each column
replace_nn_with_most_frequent <- function(geno_df) {
  # function to replace "NN" in a given column
  replace_nn_in_column <- function(column) {
    # exclude "NN" values to find the most frequent genotype
    non_missing_genotypes <- column[column != "NN"]

    # find the most frequent genotype
    if (length(non_missing_genotypes) > 0) {
      most_frequent_genotype <- names(
        sort(table(non_missing_genotypes),
          decreasing = TRUE
        )
      )[1]

      # replace "NN" with the most frequent genotype
      column[column == "NN"] <- most_frequent_genotype
    }
  ...
  geno_df <- as.data.frame(apply(geno_df, 2, replace_nn_in_column))
...
estimate_wiser_phenotype <- function(omic_df, raw_pheno_df, trait_,
                                     ...
                                     envir_var = "Country",
                                     fixed_effects_vars_computed_as_factor_by_envir = c("Latitude", "Longitude"),
...
# create a new lm function which use the pseudo inverse to deal
# with multicolinearity
lm_ <- function(formula, data, ...) {
  ...
  z <- list(
    ...
    terms = attr(mf, "terms"),
    model = mf,
    rank = qr_decomp$rank, # use qr decomposition rank
    qr = qr_decomp # add qr decomposition
  )
...
# function which compute the lsmeans in a stable way
lsmeans_ <- function(model, data) {
  
  # create factors for genotypes and environments
  data$Genotype <- factor(data$Genotype)
  data$Envir <- factor(data$Envir)
  
  # create a new data frame for predictions
  pred_data <- expand.grid(
    Genotype = levels(data$Genotype),
    Envir = levels(data$Envir)
  )

  # get the model matrix for the new data
  if (length(attr(model$terms, "variables")) > 3 ){
    new_model_matrix <- model.matrix(~ Genotype + Envir, data = pred_data)
  }else{
    new_model_matrix <- model.matrix(~ Genotype, data = pred_data)
  }

  # get the predicted values using the coefficients from the lm_ model
  pred_data$predicted_value <- new_model_matrix %*% model$coefficients

  # compute the least squares means (LS-means) using group_by()
  ls_means <- pred_data %>%
    group_by(Genotype) %>%
    summarise(
      emmean = mean(predicted_value,
        na.rm = TRUE
      ),
      .groups = "drop"
    )

  # rename columns to match lsmeans output
  colnames(ls_means) <- c("Genotype", "lsmean")

  return(as.data.frame(ls_means))
}
